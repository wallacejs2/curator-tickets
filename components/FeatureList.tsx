        ...
        <div className="bg-white rounded-lg shadow-sm border border-gray-200 overflow-hidden cursor-pointer hover:shadow-md hover:border-blue-300 transition-all" onClick={onClick}>
            <div className="p-5">
                ...
                <div className="mt-4 pt-4 border-t border-gray-100 flex items-center gap-4 flex-wrap">
                    {(feature.linkedFeatureIds?.length || 0) > 0 && <span title={`${feature.linkedFeatureIds?.length} linked feature(s)`} className="flex items-center gap-1 text-pink-600"><SparklesIcon className="w-4 h-4" /><span className="text-xs font-medium">{feature.linkedFeatureIds?.length}</span></span>}
                    {(feature.ticketIds?.length || 0) > 0 && <span title={`${feature.ticketIds?.length} linked ticket(s)`} className="flex items-center gap-1 text-yellow-600"><TicketIcon className="w-4 h-4" /><span className="text-xs font-medium">{feature.ticketIds?.length}</span></span>}
                    {(feature.projectIds?.length || 0) > 0 && <span title={`${feature.projectIds?.length} linked project(s)`} className="flex items-center gap-1 text-red-600"><ClipboardListIcon className="w-4 h-4" /><span className="text-xs font-medium">{feature.projectIds?.length}</span></span>}
                    {(feature.taskIds?.length || 0) > 0 && <span title={`${feature.taskIds?.length} linked task(s)`} className="flex items-center gap-1 text-green-600"><ChecklistIcon className="w-4 h-4" /><span className="text-xs font-medium">{feature.taskIds?.length}</span></span>}
                    {(feature.meetingIds?.length || 0) > 0 && <span title={`${feature.meetingIds?.length} linked meeting(s)`} className="flex items-center gap-1 text-blue-600"><DocumentTextIcon className="w-4 h-4" /><span className="text-xs font-medium">{feature.meetingIds?.length}</span></span>}
                    {(feature.dealershipIds?.length || 0) > 0 && <span title={`${feature.dealershipIds?.length} linked dealership(s)`} className="flex items-center gap-1 text-gray-600"><BuildingStorefrontIcon className="w-4 h-4" /><span className="text-xs font-medium">{feature.dealershipIds?.length}</span></span>}
                </div>
            </div>
